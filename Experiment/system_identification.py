...
def simulate_mass_balance(params, df, dt=10.0):
    ...
    for i in range(N - 1):
        # CO2 dynamics
        dC_in = (occ[i] * G_CO2_OCC * 1e6 + V_s[i] * (C_s[i] - C_in[i])) / V_room
        C_in[i+1] = C_in[i] + dC_in * dt
        
        # Humidity dynamics
        dW_in = (occ[i] * G_W_OCC + RHO_AIR * V_s[i] * (W_s[i] - W_in[i])) / M_air
        W_in[i+1] = W_in[i] + dW_in * dt
        
    return C_in, W_in

def simulate_thermal(params, V_room, M_air, df, dt=10.0):
    C_air, C_mass, R_ext, R_int = params
    ...
    T_m[0] = df['indoor_t'].iloc[0] # Assume mass starts at same temp
    ...
    for i in range(N - 1):
        ...
        dT_in = (q_env + q_mass + q_int + q_s) / C_air
...
def objective_phase_b(scaled_params, V_room, M_air, df, dt, scale_factors):
    params = scaled_params * scale_factors
    T_in_sim, _ = simulate_thermal(params, V_room, M_air, df, dt)
...
def run_identification(filepath):
    ...
    print("--- Phase A: Estimating V_room and M_air ---")
    
    # Scale factors: typical V_room = 100, M_air = 100
    scale_a = np.array([100.0, 100.0])
    init_scaled_a = np.array([1.0, 1.0])
    bounds_scaled_a = [(10.0/100.0, 1000.0/100.0), (10.0/100.0, 5000.0/100.0)]
    ...
    V_room_opt, M_air_opt = res_a.x * scale_a
    ...
    print("\n--- Phase B: Estimating C_air, C_mass, R_ext, R_int ---")
    # Scale factors: C_air = 100,000, C_mass = 1,000,000, R_ext = 0.1, R_int = 0.01
    scale_b = np.array([100000.0, 1000000.0, 0.1, 0.01])
    init_scaled_b = np.array([1.0, 1.0, 1.0, 1.0])
    bounds_scaled_b = [
        (1000.0/scale_b[0], 1e7/scale_b[0]),     # C_air
        (10000.0/scale_b[1], 5e7/scale_b[1]),    # C_mass
        (0.0001/scale_b[2], 5.0/scale_b[2]),     # R_ext
        (0.0001/scale_b[3], 1.0/scale_b[3])      # R_int
    ]
    
    def obj_b_scaled(scaled_params):
        return objective_phase_b(scaled_params, V_room_opt, M_air_opt, df, dt, scale_b)
        
    res_b = minimize(obj_b_scaled, init_scaled_b, bounds=bounds_scaled_b, method='L-BFGS-B')
    C_air_opt, C_mass_opt, R_ext_opt, R_int_opt = res_b.x * scale_b
    ...
    C_sim, W_sim = simulate_mass_balance([V_room_opt, M_air_opt], df, dt)
    T_sim, Tm_sim = simulate_thermal([C_air_opt, C_mass_opt, R_ext_opt, R_int_opt], V_room_opt, M_air_opt, df, dt)
