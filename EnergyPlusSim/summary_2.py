...
def load_data(run_plan_name=None):
...
def build_view2_zone(df, zone, theme=THEME):
    ...
    fig = make_subplots(rows=7, cols=1, subplot_titles=titles, specs=specs_v2, vertical_spacing=0.04, row_heights=[0.13, 0.13, 0.13, 0.13, 0.03, 0.2, 0.2])
...
def build_view4_building(df, theme=THEME):
    specs_v4 = [
        [{"type": "bar", "colspan": 4}, None, None, None, {"type": "bar", "colspan": 4}, None, None, None, {"type": "bar", "colspan": 4}, None, None, None],
        [None]*12,
        [{"type": "box", "colspan": 4}, None, None, None, {"type": "box", "colspan": 4}, None, None, None, {"type": "box", "colspan": 4}, None, None, None],
        [None]*12,
        [{"type": "xy", "colspan": 12}, None, None, None, None, None, None, None, None, None, None, None],
        [None]*12,
        [{"type": "xy", "colspan": 12}, None, None, None, None, None, None, None, None, None, None, None]
    ]
    titles = [
    ...
    fig = make_subplots(rows=7, cols=12, subplot_titles=titles, specs=specs_v4, vertical_spacing=0.04, horizontal_spacing=0.02, row_heights=[0.15, 0.005, 0.15, 0.03, 0.28, 0.01, 0.28])
    ...
    fig.add_trace(go.Bar(name='Zone Controller', x=ZONES, y=p_RH, marker_color=COLORS['Hum']), row=1, col=5)
    fig.add_trace(go.Bar(name='Zone Controller', x=ZONES, y=p_CO2, marker_color=COLORS['CO2']), row=1, col=9)

    # 4.2 Box Plots
    for z in ZONES:
        val_T_p = df[f'{z}_Temp_C']
        fig.add_trace(go.Box(y=val_T_p, name=f"{z} (Zone Controller)", marker_color=COLORS[z], showlegend=False), row=3, col=1)
        val_RH_p = df[f'{z}_RH_pct']
        fig.add_trace(go.Box(y=val_RH_p, name=f"{z} (Zone Controller)", marker_color=COLORS[z], showlegend=False), row=3, col=5)
        val_C_p = df[f'{z}_CO2_ppm']
        fig.add_trace(go.Box(y=val_C_p, name=f"{z} (Zone Controller)", marker_color=COLORS[z], showlegend=False), row=3, col=9)
...
def build_view5_energy(df, theme=THEME):
    ...
    labels_inner = ["Zone Controller"]
    values_inner = [sum(prop_totals)]
    colors_inner = [COLORS['Temp']]
    
    labels_outer = ["Fan (Zone Controller)", "Cooling (Zone Controller)", "Heating (Zone Controller)"]
    values_outer = [prop_totals[0], prop_totals[1], prop_totals[2]]
    colors_outer = [COLORS['Occ'], COLORS['Hum'], COLORS['CO2']]
    ...
    fig.add_trace(go.Pie(
        ...
        marker=dict(
            colors=colors_inner,
            line=dict(color=[pie_line_color], width=1.5)
        )
    ), row=3, col=3)
    ...
    fig.add_trace(go.Pie(
        ...
        marker=dict(
            colors=colors_outer,
            pattern=dict(shape=["-", "-", "-"]),
            line=dict(color=[pie_line_color]*3, width=1.5)
        )
    ), row=3, col=3)
...
def build_view6_summary(df, theme=THEME):
    ...
    cols_zone = ["Metric"] + [f"{z}" for z in ZONES]
    ...
    fig.add_trace(go.Table(
        header=dict(values=["End-Use", "Proposed (kWh)"], fill_color=header_bg, font=dict(color='white')),
        cells=dict(values=[
            cats + ["Total"],
...
def update_time_options(run_plan):
    if not run_plan:
        return [{"label": "Select a Run Plan First", "value": "ALL"}]
    df = load_data(run_plan)
...
def update_dashboard(view, zone, time_range, run_plan, theme):
    ...
    if not run_plan:
        ...
        return fig, True, container_style, panel_style, header_style, label_style, label_style, label_style, label_style, label_style

    df = load_data(run_plan).copy()
    if df.empty: 
        return go.Figure(), False, container_style, panel_style, header_style, label_style, label_style, label_style, label_style, label_style
    ...
    return fig, zone_disabled, container_style, panel_style, header_style, label_style, label_style, label_style, label_style, label_style
