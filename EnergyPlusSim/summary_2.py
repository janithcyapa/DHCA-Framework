...
def build_view5_energy(df):
    ...
    fig.add_trace(go.Pie(
        ...
        marker=dict(
            colors=colors_outer,
            pattern=dict(shape=["-", "-", "-"]),
            line=dict(color=['#121212', '#121212', '#121212'], width=1.5)
        )
    ), row=3, col=3)
...
def build_view6_summary(df):
    ...
    fig.add_trace(go.Table(
        header=dict(values=["End-Use", "Proposed (kWh)"], fill_color='#2c3e50', font=dict(color='white')),
        cells=dict(values=[
            cats + ["Total"],
...
@app.callback(
    ...
    [Input("view-dropdown", "value"), Input("zone-dropdown", "value"), Input("time-dropdown", "value"), Input("run-plan-dropdown", "value")]
)
def update_dashboard(view, zone, time_range, run_plan):
    if not run_plan:
        fig = go.Figure()
        fig.update_layout(
